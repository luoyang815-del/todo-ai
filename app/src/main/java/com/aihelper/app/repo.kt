...
class Repo(private val ctx: Context) {
    ...
    suspend fun exportJson(): File = withContext(Dispatchers.IO){
        ...
        dir.mkdirs()
        val f = File(dir, "AIHelperExport-${System.currentTimeMillis()}.json")
    ...
    private suspend fun refreshWidget(){
        ...
        val ids = gm.getGlanceIds(com.aihelper.app.widget.TodosWidget::class.java)
        ids.forEach { id -> com.aihelper.app.widget.TodosWidget().update(ctx, id) }
    }
