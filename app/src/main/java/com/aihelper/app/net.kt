...
interface SyncApi {
  ...
  @GET("/api/v1/sync/pull") suspend fun pull(@Header("Authorization") auth:String, @Query("since_todo") sinceTodo:Long, @Query("since_msg") sinceMsg:Long): PullResp
}
...
fun okHttpSecure(
  ...
  pinHost:String="", pinSha256:String="", trustCustomCA:Boolean=false
): OkHttpClient {
  ...
  if (pinHost.isNotBlank() && pinSha256.isNotBlank()) {
    b.certificatePinner(CertificatePinner.Builder().add(pinHost, "sha256/$pinSha256").build())
  }

  if (trustCustomCA) {
    val (ssl, tm) = customCaTrust(ctx)
