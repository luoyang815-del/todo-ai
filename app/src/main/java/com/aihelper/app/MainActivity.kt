...
fun TodoTab(onLog:(String)->Unit){
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Button(onClick = {
            scope.launch { Repo(ctx).exportJson().let { onLog("已导出: "+it.absolutePath); Noti.notify(ctx,"导出","JSON 已生成") } }
        }){ Text("导出数据") }
    }
}
...
fun ChatTab(onLog:(String)->Unit){
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        Button(onClick = {
            scope.launch { Repo(ctx).addMessage("user", text); onLog("已保存聊天（本地）") }
        }){ Text("保存") }
        Button(onClick = {
            scope.launch {
...
fun SettingsTab(onLog:(String)->Unit){
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedTextField(proxyHost, {proxyHost=it}, label={Text("代理主机")})
            OutlinedTextField(proxyPort, {proxyPort=it}, label={Text("端口")})
        }
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedTextField(proxyUser, {proxyUser=it}, label={Text("用户名")})
            OutlinedTextField(proxyPass, {proxyPass=it}, label={Text("密码")}, visualTransformation=PasswordVisualTransformation())
        }
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(onClick = {
                scope.launch {
                    ...
                    saveProxy(ctx, proxyType, proxyHost, proxyPort.toIntOrNull()?:0, proxyUser, proxyPass)
                    onLog("已保存代理/网关")
                }
            }){ Text("保存代理设置") }
        }
