...
fun ChatTab(onLog: (String) -> Unit) {
    ...
    Row(
        ...
        OutlinedTextField(
            value = pinHost,
            onValueChange = { pinHost = it },
            label = { Text("Pin 主机（可选）") },
            modifier = Modifier.weight(1f)
        )
        OutlinedTextField(
            value = pinSha256,
            onValueChange = { pinSha256 = it },
            label = { Text("Pin SHA256（去掉 sha256/ 前缀）") },
            modifier = Modifier.weight(1f)
        )
    }
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        FilterChip(
            selected = trustCA,
            onClick = { trustCA = !trustCA },
            label = { Text("信任自定义 CA（res/raw/ca.pem）") }
        )
    }
...
fun SettingsTab(onLog: (String) -> Unit) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Button(onClick = { proxyType = 2; onClick@{ proxyType = 2; onLog("代理=HTTPS") } }) { Text("HTTPS") }
        ...
        Row(
            ...
            OutlinedTextField(
                value = pinSha256,
                onValueChange = { pinSha256 = it },
                label = { Text("Pin SHA256（去掉 sha256/）") },
                modifier = Modifier.weight(1f)
            )
        }
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            FilterChip(
                selected = trustCA,
                onClick = { trustCA = !trustCA },
                label = { Text(if (trustCA) "信任自签 CA：开" else "信任自签 CA：关") }
            )

            Button(
                onClick = {
                    scope.launch {
                        saveTrustCA(ctx, trustCA)
                        onLog("已保存 CA 信任设置")
                    }
                }
            ) { Text("保存 CA 设置") }
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(onClick = { SyncWorker.start(ctx, 15); onLog("自动同步：已开启（15 分钟）") }) {
                Text("开启")
            }
            Button(onClick = { SyncWorker.stop(ctx); onLog("自动同步：已关闭") }) {
                Text("关闭")
            }
        }
    }
