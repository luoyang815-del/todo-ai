...
@Composable fun SettingsTab(onLog:(String)->Unit){
  ...
  var trustCA by remember { mutableStateOf(true) }
  ...
  Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
      OutlinedTextField(proxyHost, {proxyHost=it}, label={Text("代理主机")}); OutlinedTextField(proxyPort, {proxyPort=it}, label={Text("端口")})
    }
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
      OutlinedTextField(proxyUser, {proxyUser=it}, label={Text("用户名")}); OutlinedTextField(proxyPass, {proxyPass=it}, label={Text("密码")}, visualTransformation=PasswordVisualTransformation())
    }
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
      ...
      OutlinedTextField(pinSha256, {pinSha256=it}, label={Text("Pin SHA256（去掉 sha256/）")}, modifier=Modifier.weight(1f))
    }
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
      Button(onClick = { scope.launch { saveGateway(ctx, gateway); saveProxy(ctx, proxyType, proxyHost, proxyPort.toIntOrNull()?:0, proxyUser, proxyPass); onLog("已保存代理/网关") } }){ Text("保存代理设置") }
      Button(onClick = { trustCA = !trustCA; onLog(if (trustCA) "信任内置 CA：开" else "信任内置 CA：关") }){ Text(if (trustCA) "自签CA(开)" else "自签CA(关)") }
    }
  }
}
