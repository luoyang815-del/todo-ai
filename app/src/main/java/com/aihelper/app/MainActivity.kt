...
@Composable fun ChatTab(onLog:(String)->Unit){
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Button(onClick = {
            scope.launch {
                val proxy = proxyFlow(ctx).first(); val gateway = gatewayFlow(ctx).first(); val token = encryptedPrefs(ctx).getString("token","") ?: ""
...
@Composable fun SettingsTab(onLog:(String)->Unit){
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Button(onClick = {
                scope.launch {
                    val proxy = proxyFlow(ctx).first(); val tk = encryptedPrefs(ctx).getString("token","") ?: ""
