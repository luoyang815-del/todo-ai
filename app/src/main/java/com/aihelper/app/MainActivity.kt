...
fun TodoTab(onLog: (String) -> Unit) {
    ...
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        OutlinedTextField(
            ...
            label = { Text("标题") }, modifier = Modifier.weight(1f)
        )
        OutlinedTextField(
            value = content, onValueChange = { content = it },
            label = { Text("内容") }, modifier = Modifier.weight(1f)
        )
    }
...
fun ChatTab(onLog: (String) -> Unit) {
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        FilterChip(selected = trustCA, onClick = { trustCA = !trustCA }, label = { Text("信任自定义 CA（res/raw/ca.pem）") })
    }
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Button(onClick = {
            scope.launch {
                ...
                try {
                    val ans = Repo(ctx).chatOnce(gateway, token, proxy, model, text, pinHost, pinSha256, trustCA)
                    onLog("网关返回: " + ans.take(120))
                    Noti.notify(ctx, "聊天完成", ans.take(40))
                } catch (e: Exception) {
                    onLog("发送失败: " + (e.message ?: ""))
                }
            }
        }) { Text("发送") }
    }
...
fun SettingsTab(onLog: (String) -> Unit) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        OutlinedTextField(value = token, onValueChange = { token = it }, label = { Text("Token（仅本机）") }, visualTransformation = PasswordVisualTransformation(), modifier = Modifier.fillMaxWidth())
        ...
        OutlinedTextField(value = gateway, onValueChange = { gateway = it }, label = { Text("网关 BaseURL") }, modifier = Modifier.fillMaxWidth())

        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedTextField(value = proxyHost, onValueChange = { proxyHost = it }, label = { Text("代理主机") })
            OutlinedTextField(value = proxyPort, onValueChange = { proxyPort = it }, label = { Text("端口") })
        }
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedTextField(value = proxyUser, onValueChange = { proxyUser = it }, label = { Text("用户名") })
            OutlinedTextField(value = proxyPass, onValueChange = { proxyPass = it }, label = { Text("密码") }, visualTransformation = PasswordVisualTransformation())
        }
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(onClick = {
                scope.launch {
                    saveGateway(ctx, gateway)
                    val port = proxyPort.toIntOrNull() ?: 0
                    saveProxy(ctx, proxyType, proxyHost, port, proxyUser, proxyPass)
                    onLog("已保存代理/网关")
                }
            }) { Text("保存代理设置") }
        }
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedTextField(value = pinHost, onValueChange = { pinHost = it }, label = { Text("Pin 主机") }, modifier = Modifier.weight(1f))
