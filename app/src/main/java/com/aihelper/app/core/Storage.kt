...
object Storage {
  ...
  suspend fun loadTodos(ctx: Context) = withContext(Dispatchers.IO) {
    val f = file(ctx); if (!f.exists()) return@withContext emptyList<Todo>()
  ...
  suspend fun saveTodos(ctx: Context, todos: List<Todo>) = withContext(Dispatchers.IO) {
    val arr = JSONArray()
    todos.forEach {
      arr.put(JSONObject().put("id", it.id).put("title", it.title).put("content", it.content).put("updatedAt", it.updatedAt))
    }
  ...
  suspend fun exportJson(ctx: Context): File = withContext(Dispatchers.IO) {
    ...
    val src = file(ctx); if (src.exists()) out.writeText(src.readText()) else out.writeText("[]")
    out
  }
}
