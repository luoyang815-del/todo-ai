...
fun SettingsPage() {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("设置") }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).padding(16.dp),
            ...
            OutlinedTextField(value = proxyHost, onValueChange = { proxyHost = it }, label = { Text("代理/网关 Host") }, singleLine = true, modifier = Modifier.fillMaxWidth())
            OutlinedTextField(value = proxyPortText, onValueChange = { proxyPortText = it.filter { ch -> ch.isDigit() }.take(5) }, label = { Text("端口") }, singleLine = true, modifier = Modifier.fillMaxWidth())
            OutlinedTextField(value = proxyUser, onValueChange = { proxyUser = it }, label = { Text("用户名（可选）") }, singleLine = true, modifier = Modifier.fillMaxWidth())
            OutlinedTextField(value = proxyPass, onValueChange = { proxyPass = it }, label = { Text("密码（可选）") }, singleLine = true, modifier = Modifier.fillMaxWidth())
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp), verticalAlignment = Alignment.CenterVertically) {
                Button(onClick = {
                    val port = proxyPortText.toIntOrNull() ?: 0
                    p.edit().putString("endpoint", endpoint.trim()).putString("api_key", apiKey.trim())
                        .putString("proxy_type", proxyType).putString("proxy_host", proxyHost.trim())
                        .putInt("proxy_port", port).putString("proxy_user", proxyUser.trim())
                ...
                Button(onClick = {
                    try {
                        val port = proxyPortText.toIntOrNull() ?: 0
                        SmartOrganizer.save(context, AppSettings(endpoint.trim(), apiKey.trim(), proxyType, proxyHost.trim(), port, proxyUser.trim(), proxyPass, model))
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun ExposedDropdown(label: String, options: List<String>, selected: String, onSelected: (String) -> Unit) {
...
data class AppSettings(
    val endpoint: String, val apiKey: String, val proxyType: String,
    val proxyHost: String, val proxyPort: Int, val proxyUser: String, val proxyPass: String, val model: String
)
