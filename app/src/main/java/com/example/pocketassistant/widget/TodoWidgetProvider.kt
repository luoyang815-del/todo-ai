...
class TodoWidgetProvider: AppWidgetProvider(){
    ...
    companion object{
        ...
        fun requestUpdate(context: Context){ val mgr=AppWidgetManager.getInstance(context); val ids=mgr.getAppWidgetIds(ComponentName(context, TodoWidgetProvider::class.java)); if(ids.isNotEmpty()) updateAll(context,mgr,ids) }
        private fun updateAll(context: Context, mgr: AppWidgetManager, ids: IntArray){
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                fun scalar(sql:String):Int{ val cc=db.openHelper.readableDatabase.query(sql); cc.moveToFirst(); val v=cc.getInt(0); cc.close(); return v }
                ...
                ids.forEach { id->
                    val rv=RemoteViews(context.packageName, R.layout.widget_todo).apply{
                        setTextViewText(R.id.date, sdf.format(Date())); setTextViewText(R.id.summary, "全部:%d  重要:%d  紧急:%d".format(total,important,urgent))
