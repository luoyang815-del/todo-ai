...
@Dao interface EventDao {
  ...
  @Query("SELECT * FROM events WHERE status='open' ORDER BY CASE WHEN priority>=2 THEN 0 WHEN priority=1 THEN 1 ELSE 2 END, COALESCE(remindAt,startTime,createdAt) ASC LIMIT 10")
