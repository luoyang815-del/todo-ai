...
class MainViewModel(private val app: Application): AndroidViewModel(app){
    ...
    fun addQuickAndParse(text:String){ if(text.isBlank()) return; viewModelScope.launch{
        val entry=Entry(rawText=text, hasAiParsed=true, source="chat"); db.entryDao().insert(entry)
        val now=System.currentTimeMillis(); val event=Event(entryId=entry.id, title=text.take(30), description="（演示）智能整理结果（已创建提醒/用于小组件展示）", startTime=now+3600000, remindAt=now+600000, priority=1)
        db.eventDao().insert(event); TodoWidgetProvider.requestUpdate(app)
    } }
    companion object{ fun provideFactory(app: Application)=object: ViewModelProvider.Factory{ override fun <T:ViewModel> create(c:Class<T>):T{ @Suppress("UNCHECKED_CAST") return MainViewModel(app) as T } } }
}
