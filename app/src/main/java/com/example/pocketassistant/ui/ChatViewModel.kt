...
class ChatViewModel(private val app: Application): AndroidViewModel(app) {
    ...
    fun send(text: String) {
        viewModelScope.launch {
            ...
            val event = Event(entryId = entry.id, title = text.take(30), description = "（演示）已整理为待办", priority = 1)
            db.eventDao().insert(event)
            _messages.value = _messages.value + Message(role = "assistant", content = "已整理为待办。", hasAiParsed = true, description = event.description)
        }
    }
    companion object {
