        ...
        Column(Modifier.padding(padding).padding(16.dp)) {
            ...
            Row {
                Button(onClick = {
                    ...
                    Toast.makeText(androidx.compose.ui.platform.LocalContext.current, "已保存", Toast.LENGTH_SHORT).show()
                }) { Text("保存") }
                Spacer(Modifier.width(12.dp))
                Button(onClick = { Toast.makeText(androidx.compose.ui.platform.LocalContext.current, "测试连通：示例", Toast.LENGTH_SHORT).show() }) { Text("测试连通性") }
            }
        }
    }
}
